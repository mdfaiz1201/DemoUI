...
const Sidebar = () => {
  ...
  const renderSideItem = iconItems.map((item) => {
    return (
      <NavLink to={`/${item.name}`} className='nav text-gray-400 flex items-center px-3 rounded-md py-2 mb-4'>
        <span className='mr-4 border-gray-400 border-2 rounded-md p-1'>{item.icon}</span>
        <span>{item.name}</span>
      </NavLink>
    );
  });
  ...
  return (
    <div className='mt-0 mobile-m md:w-64 md:h-screen p-6 pb-0 bg-indigo-950 hidden-sidebar'>
